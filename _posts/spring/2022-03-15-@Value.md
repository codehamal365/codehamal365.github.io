...
所以今天有必要和大家一起，重新认识一下`@Value`。![@Value注解](assets/spring/202203191639922.png)
...
![@Value错误示例](assets/spring/202203191639512.png)此外，如果只在@Value注解中指定了系统属性名，但实际在配置文件中没有配置它，也会报跟上面一样的错。
...
![乱码问题示例](assets/spring/202203191639539.png)那么，如何解决乱码问题呢？
...
但如果项目中包含大量中文系统属性值，每次都需要加这样一段特殊转换代码，会出现大量重复代码，这显然不够优雅。
...
这种方式同样能解决乱码问题，且不会出现重复代码。但需要做一点额外的转换工作，不过这个转换非常容易，因为有现成的在线转换工具。
...
![YAML编码示例](assets/spring/202203191639766.png)
...
在定义参数时直接给个默认值，但如果仔细想想这招是行不通的。因为设置userName默认值的时机，比@Value注解依赖注入属性值要早，也就是说userName初始化好了默认值，后面还是会被覆盖。
...
这种方法确实能够完成List注入。但这主要展示了@ConfigurationProperties注解的强大，与@Value关系不大。
...
结果却跟想象中不太一样：![Set默认值错误示例](assets/spring/202203191640261.png)Set集合怎么不是空的，而是包含了一个空字符串的集合？

好吧，那我在:号后加null，总可以了吧？![Set null默认值错误示例](assets/spring/202203191640025.png)Set集合也不是空的，而是包含了一个"null"字符串的集合。
...
运行之后，结果对了：![Set空集合示例](assets/spring/202203191640790.png)
...
上面详细介绍了@Value的多种用法，归根结底就是`${}`和`#{}`的用法。
...
如前所述，YAML格式的配置文件可以避免中文乱码问题，并且语法更简洁。以下是一个使用YAML的例子：
...
### 9.2 与Spring Profiles结合使用

@Value可以与Spring Profiles结合，实现不同环境下的不同配置：
...
### 9.3 @Value vs @ConfigurationProperties

虽然@Value非常灵活，但对于复杂的配置对象，推荐使用@ConfigurationProperties：
...
在单元测试中，可以使用@SpringBootTest和@TestPropertySource来测试：

```java
@SpringBootTest
@TestPropertySource(properties = {
    "susan.test.userName=TestUser",
    "susan.test.age=20"
})
public class UserServiceTest {
    ...
    public void testValueInjection() {
        assertEquals("TestUser", userService.getUserName());
        assertEquals(20, userService.getAge());
    }
}
```
...
@Value在应用启动时进行注入，对于静态配置没有性能问题。但如果需要动态刷新配置，考虑使用@ConfigurationProperties配合@RefreshScope。

### 9.6 常见错误

1. 忘记设置默认值导致启动失败
2. 在静态变量上直接使用@Value（需要setter方法）
3. 数组类型不匹配导致转换错误
4. EL表达式语法错误
...
```java
@Value("#{'${app.env:dev}' == 'prod' ? '${db.prod.url}' : '${db.dev.url}'}")
...
```java
@Value("#{2 * ${app.thread.pool.size:10}}")
